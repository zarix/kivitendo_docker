...
# Basic Pakets
RUN apt-get update && apt install -y apache2 libarchive-zip-perl libclone-perl \
...
RUN apt install -y texlive-base-bin texlive-latex-recommended texlive-fonts-recommended \
  texlive-latex-extra texlive-lang-german texlive-xetex ghostscript
...
RUN cpan Algorithm::CheckDigits Archive::Zip CAM::PDF CGI Clone Config::Std \
  ...
  Template Text::CSV_XS Text::Iconv Text::Unidecode URI YAML::XS || :

# Kivitendo
RUN cd /var/www && \
  git clone https://github.com/kivitendo/kivitendo-erp.git && \
  cd /var/www/kivitendo-erp && \
  git checkout release-3.5.6.1 && \
